...
class Aventura {
  ...
  void jugar() {
    ...
    //------------------------------------------------
      ...
      textSize(15);
      text("Haga click", 300, 325);
    } else if (estado.equals("jugando")) {
      juego.play(aventura);
      juego.teclas();
    }
   else if ( estado.equals("perder") ) {
      text("Todo se pone oscuro a tu alrederdor, frio y lejano.", width/4, height/5);
      text("Sientes como tu cuerpo se evapora.", width/4, height/5*2);
      text("Final Jacky, haga Click", width/3, height/6*4);
    }
     if ( estado.equals("ganar") ) { 
          image( imagen[7], 400, 300, 100, 100);
          text (" Fue dificil sacarte de ahi, ten mas cuidado con que fantasmas hablas.", 150, 100);
          text (" Ahora, como favor por ayudarte me gustaria una tasa de te.", 150, 125);
        } if (estado.equals("ganar2")) {
          background(255);
          fill(0);
          textSize(15);
          image( imagen[7], 400, 300, 100, 100);
          text (" Final Mr.Phantom.", 250, 100);
        }
    //------------------------------------------------
